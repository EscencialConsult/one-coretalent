...
export default function TestRunnerShell({ evaluacion }) {
  ...
  const guardarAhora = useCallback(async (respuestasAGuardar = respuestasRef.current) => {
    if (!iniciadoRef.current || finalizadoRef.current || Object.keys(respuestasAGuardar).length === 0) {
      return false;
    }
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => guardarAhora(respuestas), 700);
    return () => clearTimeout(timeout);
  }, [fase, guardarAhora, respuestas]);
  ...
  async function comenzar() {
    ...
    try {
      ...
      const recuperadas = progreso.respuestas || {};
      setRespuestas(recuperadas);
      const primeraPendiente = adaptador.items.findIndex(
        (item) => !adaptador.respondida(item, recuperadas)
      );
      setIndice(primeraPendiente >= 0 ? primeraPendiente : 0);
  ...
  return <ShellCabecera titulo={evaluacion.test_nombre} onSalir={salir} estadoGuardado={guardado} guardadoFallido={guardadoFallido} onReintentarGuardado={guardarAhora}><div className="max-w-5xl mx-auto"><div className="tarjeta p-4 sm:p-5 mb-5"><div className="flex justify-between text-sm font-bold"><span>Progreso</span><span>{respondidas}/{total}</span></div><div role="progressbar" aria-label="Progreso de la evaluación" aria-valuemin="0" aria-valuemax={total} aria-valuenow={respondidas} className="h-2 bg-gray-100 rounded-full mt-3 overflow-hidden"><div className="runner-progress-bar h-full bg-purple-700 transition-all" style={{ width: `${total ? (respondidas / total) * 100 : 0}%` }} /></div></div><div className="grid lg:grid-cols-[1fr_260px] gap-5"><section className="tarjeta p-6 sm:p-8"><p className="text-xs uppercase text-gray-400 font-bold">Pregunta {indice + 1} de {total}</p><h2 className="text-xl font-extrabold mt-3" tabIndex="-1">{actual.texto}</h2><div className="mt-6">{adaptador.render(actual, respuestas, setRespuestas)}</div>{error && <p role="alert" className="mt-4 text-sm text-red-600">{error}</p>}<div className="flex justify-between gap-3 mt-7"><button className="btn-secundario" disabled={indice === 0} onClick={() => setIndice((x) => Math.max(0, x - 1))}>← Anterior</button>{indice < total - 1 ? <button className="btn-primario" onClick={() => setIndice((x) => Math.min(total - 1, x + 1))}>Siguiente →</button> : <button className="btn-primario" disabled={!completa || enviando} onClick={() => finalizar()}>{enviando ? "Enviando…" : "Finalizar"}</button>}</div></section><aside className="tarjeta p-5 h-fit"><h3 className="font-extrabold text-sm">Navegación</h3><div className="grid grid-cols-5 gap-2 mt-4">{adaptador.items.map((item, i) => <button aria-label={`Ir a pregunta ${i + 1}`} aria-current={i === indice ? "step" : undefined} key={item.key} onClick={() => setIndice(i)} className={`h-9 rounded-lg text-xs font-bold ${i === indice ? "bg-purple-800 text-white" : adaptador.respondida(item, respuestas) ? "bg-green-100 text-green-800" : "bg-gray-100"}`}>{i + 1}</button>)}</div></aside></div></div></ShellCabecera>;
}
...
function adaptadorSimple(items, escala) {
  ...
  return {
    ...
    render: (item, r, set) => <div className="space-y-3">{item.opciones.map((o) => <button type="button" aria-pressed={r[item.key] === o.value} key={o.value} onClick={() => set((prev) => ({ ...prev, [item.key]: o.value }))} className={`w-full text-left p-4 rounded-xl border-2 transition focus:outline-none focus:ring-2 focus:ring-purple-700 focus:ring-offset-2 ${r[item.key] === o.value ? "border-purple-700 bg-purple-50" : "border-gray-200 hover:border-purple-300"}`}>{o.label}</button>)}</div>,
  };
}
